...
# This hasn't been well maintained, and needs a little more work
# Even the config.json entries haven't been utilized
...
createml_json_file = "vs_debug_error.json"
with open(createml_json_file, "r") as fp:
    createml_json = json.load(fp)

img_file = Image.open(createml_json[0]["image"])

coco_annos = []

for i, item in enumerate(createml_json[0]["annotations"]):
    coco_anno = dict()
...
folder, filename = os.path.split(createml_json_file)
